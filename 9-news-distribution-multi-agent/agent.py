...
generate_webpage = LlmAgent(
    name="generate_webpage",
    model=GEMINI_MODEL_NAME,
    instruction=(
        "You are a helpful agent who generates a webpage from the news summaries and translations provided. "
        "You will create a well-formatted HTML page that includes the original summaries and their translations in Bahasa, Thai, and Vietnamese."
    ),
    description=(
        "Generates a webpage from the news summaries and translations provided. "
        "The webpage will include the original summaries and their translations in Bahasa, Thai, and Vietnamese."
    ),
    output_key="webpage_content",
)
root_agent = SequentialAgent(
    name="news_distribution_agent",
    sub_agents=[rss_feed_agent, parallel_pipeline, generate_webpage],
    description=(
        "A multi-agent system that fetches news from RSS feeds, summarizes them, "
        "and translates the summaries into multiple languages. If there is no RSS feed provided, do not run the translation pipeline. "        
    ),
)